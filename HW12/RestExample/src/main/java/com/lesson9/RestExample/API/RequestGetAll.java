...
public class RequestGetAll implements Request{
    ...
    public String sendRequest() throws URISyntaxException, IOException, InterruptedException {
        ...
        System.out.println("GET /clients " + uri);

        HttpClient HttpClient = java.net.http.HttpClient.newHttpClient();

        var request = HttpRequest.newBuilder(uri).GET().header("Content-type", "text/csv").build();
