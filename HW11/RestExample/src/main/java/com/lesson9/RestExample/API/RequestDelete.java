...
public class RequestDelete implements Request {
    ...
    public String sendRequest() throws URISyntaxException, IOException, InterruptedException {
        ...
        URI uri = new URI(url);
        System.out.println("DELETE /clients/{id} " + uri);

        HttpClient HttpClient = java.net.http.HttpClient.newHttpClient();

        var request = HttpRequest.newBuilder(uri).DELETE().header("Content-type", "application/json").build();
